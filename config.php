<?php
/* DATABASE SETTINGS */
// Database hostname, don't change this unless your hostname is different
define('db_host','localhost');
// Database username
define('db_user','u701780604_scm');
// Database password
define('db_pass','Tipuce57!');
// Database name
define('db_name','u701780604_scmbdd');

/* GENERAL SETTINGS */
// This will change the title on the website
define('site_name','Stay Around The World');
// Site url
define('site_url','https://sportingclubdemarly.fr');
// Featured image URL
define('featured_image','imgs/featured-image.jpg');
// Account required for checkout?
define('account_required',true);
// The from email that will appear on the customer's order details email
define('mail_from','noreply@yourwebsite.com');
// The name of your mail
define('mail_name','Service technique');
// Send mail to the customers, etc?
define('mail_enabled',true);
// Rewrite Url
define('rewrite_url',true);
?>
